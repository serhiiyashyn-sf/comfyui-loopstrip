...
- **Face detection — best path: SAM2 head_mask on ONE reference frame.** When the user pipes a SAM2-segmented head mask for the front view into `head_mask`, Center Character measures `head_height_above_feet = ref_silhouette_bottom - head_centroid_y` and applies it to every frame via `face_cy = ch - head_height_above_feet`. Character physical height is invariant across angles, so this works on back/side views where no face is visible. X uses silhouette bbox center.
- **Face detection fallback: lbpcascade_animeface** (bundled XML). Used only when `head_mask` is not provided. Heuristic approaches (fixed ratios, width profile / neck detection, skin color, DWPose, eye detection, symmetry, head-blob centroid) were tried 15+ times and ALL failed on chibi characters. The cascade is the only reliable heuristic alternative. Don't reintroduce simpler heuristics.
- **Center Character scaling**: per-image crop-based — `scale = min(target / ch, target / cw)`. Source-image-based scaling was tried and broke when characters had different padding in their source frames. Global scale (one per batch) was also tried and produced inconsistent results run-to-run because Find Best Cycle's cycle selection can change with input variations — different cycles → different max bbox → different scale.
